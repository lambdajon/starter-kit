...
import { GetAllUsers } from '../../regulars/GetAllUsers';
export default class UserRouting extends HttpRouting {
  constructor(path = '/') {
    ...
    this.router.route(`${this.prefix}/:id`).delete((req, res) => new DeleteUser().act(req, res));
    this.router.route(this.prefix).get((req, res) => new GetAllUsers().act(req, res));
